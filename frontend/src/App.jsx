...
//     const handleFileChange = (e) => { setFile(e.target.files[0]); setError(''); setUploadResults(null); };
//     const handleSubmit = async (e) => {
//         e.preventDefault();
//         if (!file) { setError('Veuillez sélectionner un fichier à télécharger.'); return; }
//         setIsUploading(true); setError(''); setUploadResults(null);
//         const formData = new FormData();
//         formData.append('file', file);
...
//             const data = await response.json();
//             if (response.ok) setUploadResults(data);
//             else setError(data.detail || "Échec du téléchargement du fichier.");
//         } catch (err) { setError('Une erreur inattendue est survenue.'); } 
//         finally { setIsUploading(false); }
//     };

//     return (
//         <div className="form-container" style={{ maxWidth: 'none', margin: 0, padding: '2rem' }}>
//             <h3>Ajouter un Passeport via Téléchargement</h3>
//             <p className="mb-2" style={{ color: 'var(--secondary-color)' }}>Téléchargez une image ou un PDF. Le système traitera chaque page individuellement.</p>
//             {error && <p className="error-message">{error}</p>}
//             {!uploadResults ? (
//                 <form onSubmit={handleSubmit}>
//                     <div className="form-group">
//                         <label>Destination (Optionnel)</label>
//                         <input type="text" name="destination" value={destination} onChange={(e) => setDestination(e.target.value)} className="form-input" list="destination-datalist-ocr" placeholder="Appliquer à tous les passeports de ce fichier" autoComplete="off" />
//                         <datalist id="destination-datalist-ocr">{destinations.map(dest => <option key={dest} value={dest} />)}</datalist>
//                     </div>
//                     <div className="form-group">
//                         <label>Document de Passeport</label>
//                         <input type="file" onChange={handleFileChange} accept="image/png, image/jpeg, application/pdf" className="form-input" required disabled={isUploading} />
//                     </div>
//                     <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '1rem', marginTop: '2rem' }}>
//                         <button type="button" onClick={onUploadSuccess} className="btn" style={{ backgroundColor: 'var(--secondary-color)', color: 'white' }} disabled={isUploading}>Annuler</button>
//                         <button type="submit" className="btn btn-primary" disabled={isUploading || !file}>{isUploading ? 'Traitement...' : 'Télécharger et Extraire'}</button>
//                     </div>
//                 </form>
//             ) : (
//                 <div className="results-container">
//                     <p className="results-summary">Traitement Terminé : {uploadResults.successes.length} réussis, {uploadResults.failures.length} échoués.</p>
//                     <ul className="results-list">
//                         {uploadResults.successes.map(success => (
//                             <li key={success.id} className="result-success">
//                                 <span className="result-icon"><SuccessIcon /></span>
//                                 <span><b>Page {success.page_number}</b> traitée : Passeport créé pour <b>{success.first_name} {success.last_name}</b> (N° : {success.passport_number}).</span>
//                             </li>
//                         ))}
//                         {uploadResults.failures.map((failure, index) => (
//                             <li key={index} className="result-failure">
...
//         try {
//             // --- MODIFICATION: Changed to POST and new endpoint ---
...
function OcrUploader({ token, onUploadSuccess }) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (response.ok) {
                setUploadResults(data);
                // We call onUploadSuccess here if there are any successful uploads
                // to trigger a data refresh in the main view immediately.
                if (data.successes && data.successes.length > 0) {
                    onUploadSuccess(true); // pass a flag to indicate not to close the modal
                }
            }
            else setError(data.detail || "Échec du téléchargement du fichier.");
        } catch (err) { setError('Une erreur inattendue est survenue.'); } 
    ...
    const handleFinish = () => {
        // This function now fully delegates to onUploadSuccess to close the view
        onUploadSuccess(false); // pass false to indicate we should close
    }
        ...
        <div className="form-container" style={{ maxWidth: 'none', margin: 0, padding: '2rem' }}>
            ...
            {!uploadResults ? (
                <form onSubmit={handleSubmit}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '1rem', marginTop: '2rem' }}>
                        <button type="button" onClick={handleFinish} className="btn" style={{ backgroundColor: 'var(--secondary-color)', color: 'white' }} disabled={isUploading}>Annuler</button>
                        <button type="submit" className="btn btn-primary" disabled={isUploading || !file}>{isUploading ? 'Traitement...' : 'Télécharger et Extraire'}</button>
                    </div>
                </form>
            ) : (
                <div className="results-container">
                    ...
                    <ul className="results-list">
                        {uploadResults.successes.map(success => (
                            <li key={success.passport.id} className="result-success">
                                <span className="result-icon"><SuccessIcon /></span>
                                <span><b>Page {success.page_number}</b> traitée : Passeport créé pour <b>{success.passport.first_name} {success.passport.last_name}</b> (N° : {success.passport.passport_number}).</span>
                            </li>
                        ))}
                        {uploadResults.failures.map((failure, index) => (
                            <li key={index} className="result-failure">
                    ...
                    <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '1rem', marginTop: '2rem' }}>
                        <button type="button" onClick={handleFinish} className="btn btn-primary">Terminé</button>
                    </div>
                </div>
            )}
        </div>
